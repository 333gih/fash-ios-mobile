...
/// OkHttp [SecuredApiClient] equivalent — Bearer injection + 401 refresh retry.
final class SecuredApiClient {
    ...
    private func authorized(_ request: URLRequest) -> URLRequest {
        ...
        if !secret.isEmpty {
            req.setValue(secret, forHTTPHeaderField: "X-Internal-Secret")
        }
    ...
    private func refreshTokenIfNeeded() async throws -> Bool {
        ...
        let result = await authRepository.refresh(refreshToken: session.refreshToken)
        switch result {
